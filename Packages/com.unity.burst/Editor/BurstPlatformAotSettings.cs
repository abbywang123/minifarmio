    ...
    class BurstPlatformAotSettings : ScriptableObject
    {
        ...
        internal static BurstPlatformAotSettings GetOrCreateSettings(BuildTarget? target)
        {
            ...
            if (target.HasValue)
            {
                ...
                {
                   if (GetAttr.GetCustomAttribute<BurstCommonSettingAttribute>(field, false) != null)
                    {
        ...
        private static bool ShouldSerialiseOut(BuildTarget? target, FieldInfo field)
        {
            ...
            var hasCommonAttribute = GetAttr.GetCustomAttribute<BurstCommonSettingAttribute>(field, false) != null;
        ...
        class BurstAotSettingsProvider : SettingsProvider
        {
            ...
            private void InitialiseSettingsForPlatform(int platform, FieldInfo[] platformFields)
            {
                ...
                for (int i = 0; i < platformFields.Length; i++)
                {
                    ...
                    var displayName = GetAttr.GetCustomAttribute<DisplayNameAttribute>(
                      typeof(BurstPlatformAotSettings).GetField(platformFields[i].Name + "_DisplayName", BindingFlags.Static | BindingFlags.NonPublic),
                     false)?.DisplayName;
                    var toolTip     = GetAttr.GetCustomAttribute<DescriptionAttribute>(                       typeof(BurstPlatformAotSettings).GetField(platformFields[i].Name + "_ToolTip", BindingFlags.Static | BindingFlags.NonPublic),
                       false)?.Description;
